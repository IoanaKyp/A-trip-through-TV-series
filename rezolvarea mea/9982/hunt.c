...
uint16_t find_spell(uint64_t memory)
{
    ...
    uint8_t check_lenght=0;
    //variabila folosita pentru a verifica lungimea secventei de 1

    while(check_lenght<5)
    {
        if((memory&1) == 1)
            check_lenght++;
        else
...
uint32_t choose_sword(uint16_t enemy)
{
    ...
    uint8_t i; //variabila pentru structurile repetitive
    uint8_t det_type=0; // variabila pt determinarea tipului de inamic
    i=0;
    uint16_t value; // value din enunt
    while (i<16)
    {
        if( (enemy & (1<<i)) != 0 )
            det_type++;
    ...
    //analiza fiecarui bit in parte pentru a determina cati sunt activi
    if (det_type%2==0) //enemy --> monster
    {
        value = enemy&(1-enemy);
        res = (9<<28) + value; // 1001 in baza 2 = 9 in baza 10 si 28=32-4
    }
    else // enemy --> human
    {
        value = -enemy;
        res = (6<<28) + value; // 0110 in baza 2 = 6 in baza 10 si 28=32-4
    }
...
uint8_t trial_of_forrest_eyes(uint64_t map)
{
    ...
    if(map == UINT64_MAX)
        res = 2; 
    else if(map == (uint64_t)3<<31)
        res = 1;
    else if (map%15 == 0) // 15 = 1111 in baza 2
        res = 0;
    else 
        res = 3;
...
uint8_t trial_of_the_dreams(uint32_t map)
{
    ...
    uint8_t res = -1;
    uint8_t location_1=0, location_2=0,i=1;

    while(i<=64)
    {
        if((map&((uint64_t)1<<(i-1))) != 0 ) 
        {
            if(location_1==0)
                location_1=i;
